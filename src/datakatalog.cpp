...
///////////////////////////////
    ...
    m_blNazadSpisok = true;//true - разрешено назад к Списку.
    m_blNazadElement = true;//true - разрешено назад к Элементу.
...
/////////////////////////////////////////////////
    ...
    if(!untSpisokMax){//ВАЖНО! Если = 0, то читаем из БД, Но при последнем элементе Списка ОБНУЛЯЕМ!!!
        ...
        if(!untSpisokMax){//Если ноль элементов Списка, то...
            ...
            m_blNazadSpisok = false;//Не нужно вверх по каталогу на этом этапе...
...
/////////////////////////////////////////////////////
    ...
    if(!untElementMax){//ВАЖНО! Если = 0, то читаем из БД, Но при последнем Элементе в Списке ОБНУЛЯЕМ!!!
        ...
        if(m_pdbElement->SELECT(strImyaTablici)){//Если есть такая таблица, то....
            ...
            if(!untElementMax){//Если ноль элементов, то...
                m_blElementMax = true;//Последний Элемент в Списке.
                m_blDataEmpty = true;//Пустые данные.
                m_blNazadElement = false;//Не нужно вверх по каталогу на этом этапе...
                slotCopyDannie(true);//Слот статуса скопированного документа.
                return true;//Успех, выходим из метода.
            }
        }
    ...
    if((untElementNomer > untElementMax)||(untElementNomer <= 0)){//Если Номер не в эти рамках, то...
        ...
        if(untElementNomer == untElementMax){//Если равняется, значит это последний Элемент в Списке
            m_blElementMax = true;//Последний Элемент в Списке.
...
/////////////////////////////////////
    ...
    if(m_blNazadSpisok){//Если вверх можно перемещаться, то...
        if(nazadElement()){//Если переход на одну папку вверх удачный, то...
            if(!nazadElement())//Если повторый переход на папку выше неудачный, то...
                return false;//Ошибка
        }
        else//Если переход неудачный, то...
            return false;//Ошибка
    }
    else//Если вверх нельзя перемещаться, то....
        m_blNazadSpisok = true;//Сбрасываем флаг. Можно вверх перемещаться.

    return true;//Успех.
}
bool DataKatalog::nazadElement(){//Переходим назад в папку в Элементами.
///////////////////////////////////////
//---Н А З А Д   В   Э Л Е М Е Н Т---//
///////////////////////////////////////

    if(m_blNazadElement){//Если вверх можно перемещаться, то...
        if(!m_pdrPut->cd("..")){//Если не получилось перейти в папку назад к Элементам, то...
            qdebug(tr("Ошибка прехода назад в папку Элемента."));
            return false;//Ошибка.
        }
    }
    else//Если вверх нельзя перемещаться, то....
        m_blNazadElement = true;//Сбрасываем флаг. Можно вверх перемещаться.
    ...
    if(blCopyStatus){//Если копирование прошло успешно, то...
        ...
        if(m_blDannieMax){//Если это последние данные в конкретном Элементе, то...
            ...
            if(m_blSpisokMax && m_blElementMax){//Если, то это окончание копирования.
                ...
                else{//Это последние данные в конкрктном Элементе...
