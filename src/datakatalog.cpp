...
DataKatalog::DataKatalog(const QString strWorkingData, const QString strDomPut, QObject* proditel)
    : QObject{proditel}{
///////////////////////////////
...
///////////////////////////////
    ...
    m_strDomPut = strDomPut;//Присваиваем путь в котором будет создавться каталог документов.
    m_pdrPut = new QDir (m_strDomPut);//Путь дериктории Документы, в которой будет создаваться каталог.
    ...
    m_strFileDialogPut = "";//Путь к каталогу, где лежат файлы для открытия
...
void DataKatalog::ustFileDialogPut(QString strFileDialogPut){//Задать путь к каталогу, в котором файл.
///////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////
    ...
    if(m_untDannie){//Если в БД записан хоть один документ, то...
        m_pdrPut->setPath(m_strDomPut);//Всегда обнуляем путь на заданную по умолчание папку.
...
void DataKatalog::ustDomPut(const QString strDomPut){//Задаём путь папки, где сохранятся каталоги документов.
///////////////////////////////////////////////////////////
//---У С Т А Н О В И Т Ь   Д О М А Ш Н Ю Ю   П А П К У---//
///////////////////////////////////////////////////////////

    if(m_strDomPut != strDomPut)//Если пути не одинаковые, то...
        m_strDomPut = strDomPut;//Сохраняем новый путь.
}
...
/////////////////////////////////////////////////////////////////////////////
    ...
    m_untSpisokMax = 0;//Переменная Максимального количества элементов Списка.
    m_untElementMax = 0;//Переменная Максимального количества Элементов в Списке.
    m_untDannieMax = 0;//Переменная Максимального количества Данных в Элементе.

    m_untMaxSpisok = 0;//Максимальное количество Элементов в конкретном Списке.
    ...
    m_strTitul = "";//Имя создаваемой папки Титула.
    ...
    m_blStopCopy = false;//Разрешаем копировать документы в каталоге.
}
...
/////////////////////////////////////////////////
    m_strTitul = m_pdbTitul->SELECT("Код", "1", "Титул");//Читаем имя Титула из БД.
    if(m_strTitul.isEmpty()){//Если Титул пустой, то...
        qdebug(tr("Ошибка, имя Титула не может быть пустым."));
        return false;//Ошибка.
    }
    else{//Если имя Титула не пустое, то...
        if(m_pdrPut->mkdir(m_strTitul)){//Если папка с именем Титула создалась, то...
            if(m_pdrPut->cd(m_strTitul)){//Если получилось перейти в папку с именем Титула, то...
                m_untCd -= 1;//Минус слой, переходим в папку Титула, где Списки.
                if(!sozdatOpisanie())//Описание Списка не создалось ,то...
                    qdebug(tr("Ошибка создания описания Титула."));
            }
...
/////////////////////////////////////////////////
    ...
    if(!m_untSpisokMax){//ВАЖНО! Если = 0, то читаем из БД, Но при последнем элементе Списка ОБНУЛЯЕМ!!!
        m_untMaxSpisok = m_untSpisokMax = m_pdbSpisok->SELECTPK();//Подсчитываем количество элементов в Списке
        if(!m_untSpisokMax){//Если ноль элементов Списка, то...
    ...
    if((untSpisokNomer > m_untSpisokMax)||(untSpisokNomer <= 0)){//Если Номер не в эти рамках, то...
        ...
        for(uint untShag = untSpisokNomer; untShag <= m_untSpisokMax; untShag++){
            m_untSpisokNomer = untShag;
            if(untShag == m_untSpisokMax){//Если равняется, значит последний элемент Списка.
                m_blSpisokMax = true;//Последний элемент Списка.
                m_untSpisokMax = 0;//ОБЯЗАТЕЛЬНО ОБНУЛЯЕМ!!!
            }
                ...
                if(m_pdrPut->mkdir(strSpisok)){//Если папка с именем Списка создалась, то...
                    if(m_pdrPut->cd(strSpisok)){//Если получилось перейти в папку с именем Списка, то...
                        ...
                        if(!sozdatOpisanie(m_untSpisokKod))//Описание Списка не создалось ,то...
                            qdebug(tr("Ошибка создания описания списка."));
...
/////////////////////////////////////////////////////
    ...
    if(!m_untElementMax){//ВАЖНО! Если = 0, то читаем из БД, Но при последнем Элементе в Списке ОБНУЛЯЕМ!!!
        for(uint untShagSpiska = m_untSpisokKod; untShagSpiska <= m_untMaxSpisok; untShagSpiska++){//Цикл.
            ...
            if(m_pdbElement->SELECT(strImyaTablici)){//Если есть такая таблица, то....
                ...
                m_untElementMax = m_pdbElement->SELECTPK();//Считаем количество Элементов в каждом из Списков.
                if(m_untElementMax)//Если не ноль элементов, то...
                    untShagSpiska = (m_untMaxSpisok+1);//Выходим из цикла и передаём в метод m_untSpisokMax.
                else//Если 0, то...
    ...
    if(m_untElementNomer == 0){//Если 0 равняется, то такой таблицы или нет или в ней 0 элементов.
        m_blElementMax = true;//Последний Элемент в Списке.
        m_untElementMax = 0;//ОБНУЛЯЕМ ОБЯЗАТЕЛЬНО!!!
        return true;//Выходим из метода с Успехом.
    }
    if((m_untElementNomer > m_untElementMax)||(m_untElementNomer < 0)){//Если Номер не в эти рамках, то...
        qdebug(tr("Не правильной номер элемента для его создания."));
        return false;//Ошибка.
    }
    else{//Если нет, то...
        for(uint untShag = m_untElementNomer; untShag <= m_untElementMax; untShag++){
            m_untElementNomer = untShag;
            if(untShag == m_untElementMax){//Если равняется, значит это последний Элемент в Списке
                m_blElementMax = true;//Последний Элемент в Списке.
                m_untElementMax = 0;//ОБНУЛЯЕМ ОБЯЗАТЕЛЬНО!!!
            }
                ...
                if(m_pdrPut->mkdir(strElement)){//Если папка с именем Элемента создалась, то...
                    if(m_pdrPut->cd(strElement)){//Если получилось перейти в папку с именем Элемента, то...
                        ...
                        if(!sozdatOpisanie(m_untSpisokKod, m_untElementKod))//Описание Элемента не создалос,то
                            qdebug(tr("Ошибка создания описания элемента."));
...
///////////////////////////////////////////////////
    ...
    if(!m_untDannieMax){//ВАЖНО! Если = 0, то читаем из БД, Но при последних Данных в Элементе ОБНУЛЯЕМ!!!
        ...
        if(m_pdbDannie->SELECT(strImyaTablici)){//Если есть такая таблица, то...
            ...
            if(!m_untDannieMax){//Если ноль данных, то...
                m_blDannieMax = true;//Последние Данные в Элементе.
                m_blDataEmpty = true;//Пустые данные.
                slotCopyDannie(true);//Слот статуса скопированного документа.
                return true;//Успех, выходим из метода.
    ...
    if((untDannieNomer > m_untDannieMax)||(untDannieNomer < 0)){//Если Номер не в эти рамках, то...
        ...
        if(untDannieNomer == 0){//Если 0, то это мы обрабатываем план.pdf
            ...
            if(untDannieNomer == m_untDannieMax){//Если равняется, значит это последние Данные в Элементе.
                m_blDannieMax = true;//Последние Данные в Элементе.
                m_untDannieMax = 0;//ОБНУЛЯЕМ ОБЯЗАТЕЛЬНО!!!
            }
...
bool DataKatalog::sozdatOpisanie(){//Создаём ОПИСАНИЕ Титула.txt
///////////////////////////////////////////////////////
//---С О З Д А Т Ь   О П И С А Н И Е   Т И Т У Л А---//
///////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    if(blCopyStatus && !m_blStopCopy){//Если копирование прошло успешно, и нет запрета на копирование, то...
        ...
        if(m_blDannieMax){//Если это последние данные в конкретном Элементе, то...
            ...
            if(m_blSpisokMax && m_blElementMax){//Если, то это окончание копирования.
                ...
                qdebug(tr("Создание каталога \'")	+ m_strMentor
                       + "\' завершено. Каталог расположен в [" + m_strDomPut + "].");
