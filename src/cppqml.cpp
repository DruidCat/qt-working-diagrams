...
void DCCppQml::setStrKatalogPut(const QString &strKatalogPut){//Изменяем путь сохранения каталога.
 ////////////////////////////////////////////////////
//---И З М Е Н Я Е М   П У Т Ь   К А Т А Л О Г А---//
/////////////////////////////////////////////////////

    if(strKatalogPut.isEmpty()){//Если путь каталога пустой, то...
        qdebug(tr("Путь к сохранению каталога документов пустой."));
    }
    else{//Если нет, то...
    ...
    if(m_pdcclass->isEmpty(strTitulNovi))//Если пустая строка, то...
        ...
        if(m_strTitul != strTitulRed){//Если имена титулов не совпадают, то...
            if(m_pDataTitul->renTitul(strTitulRed)){//Если имя Титула записалось успешно, то...
                qdebug(tr("Переименован титул: ") + m_strTitul + "->" + strTitulRed);//Запись в логи.
	...
	if(m_pdcclass->isEmpty(strSpisokNovi))//Если пустая строка, то...
        ...
        for(int ntShag = 0; ntShag<slsSpisok.size(); ntShag++){//Проверка на одинаковые имена элементов
            if(slsSpisok[ntShag] == strSpisokRed){
                qdebug(tr("Список с таким именем уже существует."));
				return;
			}
		}
        if(m_pDataSpisok->ustSpisok(strSpisokRed)){//Если элемент списка записался успешно, то...
            qdebug(tr("Создан список: ") + strSpisokRed);//В логи запись.
        	emit strSpisokDBChanged();//Излучаем сигнал об изменении аргумента.
		}
	}
}
	...
	if(m_pdcclass->isEmpty(strElementNovi))//Если пустая строка, то...
        ...
        for(int ntShag = 0; ntShag<slsElement.size(); ntShag++){//Проверка на одинаковые имена элементо
            if(slsElement[ntShag] == strElementRed){
                qdebug(tr("Элемент с таким именем уже существует."));
				return;
			}
		}
        if(m_pDataElement->ustElement(m_ullSpisokKod, strElementRed)){//Если элемент списка записался успешно
            qdebug(tr("Создан элемент: ") + strElementRed);//В логи запись
    ...
    if(m_pdcclass->isEmpty(strImyaFaila)){//Если пустая строка, то...
        ...
        for(int ntShag = 0; ntShag<slsDannie.size(); ntShag++){//Проверка на одинаковые имена Данных
            if(slsDannie[ntShag] == strDannie){
                qdebug(tr("Документ с таким именем уже существует."));
				slotFileDialogCopy(false);//Ошибка копирования, отключаем анимацию копирования и всё остальное
                return;
            }
        }
        ...
        if(m_pDataDannie->ustDannie(m_ullSpisokKod, m_ullElementKod, strImyaFaila, strDannie))//Копир. Успех.
            qdebug(tr("Добавлен документ: ") + strDannie);
    }
}
...
///////////////////////////////////////////////
    if(m_pDataPlan->ustPlan(m_ullSpisokKod, m_ullElementKod, strImyaFaila)){//Если успешно скопировался план.
        qdebug(tr("Добавлен план: ") + strImyaFaila);
        return true;//Успех.
    }
    return false;//Ошибка.
}
...
QString DCCppQml::qtVersion() const {//Метод возвращающий версию Qt
/////////////////////////////////////////////
//---П О Л У Ч И Т Ь   В Е Р С И Ю   Q T---//
/////////////////////////////////////////////
    return qVersion();//воззвращаем строку с версией Qt
}
void DCCppQml::setStrDebug(QString& strDebugNovi){//Установить Новую ошибку.
///////////////////////////////////////////////////////
//---У С Т А Н О В И Т Ь   Т Е К С Т   О Ш И Б К И---//
///////////////////////////////////////////////////////
    QString strDebugDataVremya("");//Переменная, которая хранит отладочную информацию для записи в логи.
    ...
    if(strDebugNovi == "")//Стераем сообщение из Toolbar
        ...
        strDebugDataVremya = strData + "-" + strVremya + " " + strDebugNovi;//Добавляем двоеточие и само Сообщение.
        m_pTimerDebug->start();//Запустить таймер.
    }
	m_strDebug = strDebugNovi;
    if(!m_strDebug.isEmpty()){//Если не пустая строка, то...
        m_pdclogger->ustDebug(strDebugDataVremya);//Добавляем строку для её дальнейшей записи в файл.
        m_pdclogger->start();//Запускаем в отдельном потоке запись логов в файл.
        qWarning()<<strDebugDataVremya;//Пишем ошибку в отладочную консоль.
    }
...
bool DCCppQml::isPdfPoiskPusto(const QString strPoisk){//Пустой запрос на поиск?
///////////////////////////////////////////////////
//---П У С Т О Й   З А П Р О С   П О И С К А ?---//
///////////////////////////////////////////////////
	return m_pdcclass->isEmpty(strPoisk);//Вернёт true или false.
}
bool DCCppQml::isPdfPoiskTri(const QString strPoisk){//В запросе три и более символов?
///////////////////////////////////////////////
//---Т Р И   С И М В О Л А   И   Б О Л Е Е---//
///////////////////////////////////////////////

    QString strPoiskProvereni = m_pdcclass->udalitPustotu(strPoisk);
    return strPoiskProvereni.length() > 2 ? true : false;//Если больше двух символов, то истина, иначе ложь.
}
...
/////////////////////////////////////////////////////////
    ...
    m_pDataKatalog->copyStop();//Останавливаем создание каталога.
}
