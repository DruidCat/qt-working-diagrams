import QtQuick //2.15
//import QtQuick.Controls // для WheelHandler
...
Item {
	...
	PathView {//Представление модели с бесконечным скролингом.
        ...
        function activateCurrentItem() {
            if (typeof model.get === "function") {//Если model объект типа ListModel (у него есть функция get)
                var vrItem = model.get(currentIndex)//Получаем текущий элемент модели по индексу currentIndex
                if (vrItem)//Если элемент найден, то...
                    root.clicked(vrItem.spisok)//Вызываем сигнал clicked и передаём значение spisok
            }
            else {//Если не ListModel, то...
                if (Array.isArray(model)) {//Если model — это обычный массив (Array)
                    let ltItem = model[currentIndex]//Получаем текущий элемент массива по индексу currentIndex
                    if (ltItem)//Если элемент найден, то...
                        root.clicked(ltItem.spisok)//Вызываем сигнал clicked и передаём значение spisok
                }
            }
        }
    ...
    WheelHandler {//Для прокрутки модели колесиком мыши
        id: wheelHandler
